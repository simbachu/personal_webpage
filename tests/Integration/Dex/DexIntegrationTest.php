...
class DexIntegrationTest extends TestCase
{
    ...
    public function testSingleTypePokemonIntegration(): void
    {
        ...
        try {
            ...
            $rendered = $this->twig->render($viewData["template"]->toTwigPath(), array_merge(\Tests\Support\TwigTestFactory::layoutGlobals(), $viewData));
    ...
    public function testOgMetaTagsAreGeneratedForPokemonWithOpinion(): void
    {
        //! @section Arrange
        $cacheDir = $this->testCacheDir . '_og_meta';
        @mkdir($cacheDir, 0777, true);

        $pokemonJson = PokemonTestData::getPikachuJson();
        $service = new PokeApiService(function (string $url) use ($pokemonJson): string {
            return $pokemonJson;
        });

        // Create a mock opinion service that returns an opinion for pikachu
        $opinionService = $this->createMock(PokemonOpinionService::class);
        $opinionService->method('getOpinion')->willReturn(\App\Shared\Support\Result::success([
        ...
        try {
            //! @section Act
            $monsterData = $presenter->fetchMonsterData(\App\Dex\MonsterIdentifier::fromString('pikachu_og'));
            $viewData = $presenter->present($monsterData);

            // Add the meta data that would be generated by DexRouteHandler
            $viewData['meta'] = [
                'title' => 'Pikachu #25',
                'description' => 'Pokédex entry for Pikachu',
                'og_title' => 'Pikachu #25 - Jennifer\'s Rating: A',
                'og_description' => 'My sister caught a wild Pikachu in Viridian Forest.',
                'og_image' => 'https://img.example/pikachu.png',
                'og_image_alt' => 'Pikachu - Pokemon #25',
            ];

            $rendered = $this->twig->render($viewData["template"]->toTwigPath(), array_merge(\Tests\Support\TwigTestFactory::layoutGlobals(), $viewData));

            //! @section Assert
            $this->assertStringContainsString('<meta property="og:title" content="Pikachu #25 - Jennifer&#039;s Rating: A">', $rendered);
            $this->assertStringContainsString('<meta property="og:description" content="My sister caught a wild Pikachu in Viridian Forest.">', $rendered);
            $this->assertStringContainsString('<meta property="og:image" content="https://img.example/pikachu.png">', $rendered);
            $this->assertStringContainsString('<meta property="og:image:alt" content="Pikachu - Pokemon #25">', $rendered);

            // Twitter cards should also use the OG data
            $this->assertStringContainsString('<meta name="twitter:title" content="Pikachu #25 - Jennifer&#039;s Rating: A">', $rendered);
            $this->assertStringContainsString('<meta name="twitter:description" content="Pokédex entry for Pikachu">', $rendered);
        } finally {
            if (is_dir($cacheDir)) {
                array_map('unlink', glob($cacheDir . '/*') ?: []);
                @rmdir($cacheDir);
